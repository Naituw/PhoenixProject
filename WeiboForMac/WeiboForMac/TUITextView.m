...
- (void)_updateDefaultAttributes
{
	...
	renderer.markedAttributes = [NSDictionary dictionaryWithObjectsAndKeys:
						[NSFont fontWithName:self.font.fontName size:self.font.pointSize], kCTFontAttributeName,
						[self.textColor CGColor], kCTForegroundColorAttributeName,
						ABNSParagraphStyleForTextAlignment(textAlignment), NSParagraphStyleAttributeName,
						nil];
}
...
- (void)_checkSpelling
{	
	lastCheckToken = [[NSSpellChecker sharedSpellChecker] requestCheckingOfString:self.text range:NSMakeRange(0, [self.text length]) types:(NSTextCheckingTypes)NSTextCheckingTypeSpelling options:nil inSpellDocumentWithTag:0 completionHandler:^(NSInteger sequenceNumber, NSArray *results, NSOrthography *orthography, NSInteger wordCount) {
