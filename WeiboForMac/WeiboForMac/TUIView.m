...
- (CGContextRef)_CGContext
{
	...
	if(_context.context) {
		   ...
		   o != _context.lastOpaque) 
		{
			CGContextRelease(_context.context);
			_context.context = NULL;
		}else if(AtLeastLion && fabs(self.layer.contentsScale - _context.lastContentsScale) > 0.1f){
            CGContextRelease(_context.context);
			_context.context = NULL;
        }
	}
...
- (void)displayLayer:(CALayer *)layer
{
    ...
    BOOL drawInBackground = self.drawInBackground;
...
#define POST_DRAW \
    ...
    if(AtLeastLion)CGContextScaleCTM(context, 1.0f / self.layer.contentsScale, 1.0f / self.layer.contentsScale); \
	TUIGraphicsPopContext(); \
    if(drawInBackground) [CATransaction flush];
