...
- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView
{
	...
	// Draw gradient when in recording mode
		...
		// Draw snapback image
            ...
            imageRect.size = snapBackArrow.size;
            [snapBackArrow drawAtPoint:[self _snapbackRectForFrame: cellFrame].origin fromRect:imageRect operation:NSCompositeSourceOver fraction:1.0f];
	...
	// Draw border and remove badge if needed
		...
		// If key combination is set and valid, draw remove image
			if (![self _isEmpty] && [self isEnabled])
			{
                ...
                NSRect imageRect = NSZeroRect;
                imageRect.size = removeImage.size;
                [removeImage drawAtPoint:[self _removeButtonRectForFrame: cellFrame].origin fromRect:imageRect operation:NSCompositeSourceOver fraction:1.0f];
            }
		}
...
//		NSLog(@"stroked along path of %@", NSStringFromRect(correctedSnapBackRect));
		...
		// Draw snapback image
            ...
            NSRect imageRect = NSZeroRect;
            imageRect.size = snapBackArrow.size;
            [snapBackArrow drawAtPoint:correctedSnapBackOrigin fromRect:imageRect operation:NSCompositeSourceOver fraction:1.0f*alphaRecording];
		}
	...
	// Draw border and remove badge if needed
		...
		if (![self _isEmpty] && [self isEnabled])
		{
            ...
            NSRect imageRect = NSZeroRect;
            imageRect.size = removeImage.size;
            [removeImage drawAtPoint:[self _removeButtonRectForFrame: cellFrame].origin fromRect:imageRect operation:NSCompositeSourceOver fraction:alphaView];
