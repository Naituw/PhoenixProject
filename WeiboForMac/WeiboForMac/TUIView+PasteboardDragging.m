...
- (void)pasteboardDragMouseDragged:(NSEvent *)event
{
	if(!_viewFlags.pasteboardDraggingIsDragging) {
		...
		TUIImage *dragImage = TUIGraphicsDrawAsImageWithScale(dragView.frame.size, dragView.layer.contentsScale, ^{
			[TUIGraphicsGetImageForView(dragView) drawAtPoint:CGPointZero blendMode:kCGBlendModeNormal alpha:0.75];
		});
		
		NSImage *dragNSImage = [[[NSImage alloc] initWithCGImage:dragImage.CGImage size:dragView.frame.size] autorelease];
