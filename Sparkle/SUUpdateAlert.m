...
- (void)endWithSelection:(SUUpdateAlertChoice)choice
{
    ...
    [releaseNotesView setUIDelegate:nil];
    [releaseNotesView setEditingDelegate:nil];
...
- (void)displayReleaseNotes
{
    ...
    [releaseNotesView setUIDelegate:self];
    [releaseNotesView setEditingDelegate:self];
	...
	if ([updateItem releaseNotesURL])
	{
		...
		{
            // Add current host version here, so we can display all the release note between current version and newest version.
            
            NSURL * releaseNotesURL = [updateItem releaseNotesURL];
            NSString * query = [NSString stringWithFormat:@"host=%@",[host version]];
            
            NSString * URLString = [[NSString alloc] initWithFormat:@"%@%@%@", releaseNotesURL,
                                   [releaseNotesURL.absoluteString rangeOfString:@"?"].length > 0 ? @"&" : @"?", query];
